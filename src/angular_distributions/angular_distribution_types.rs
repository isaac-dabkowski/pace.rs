...
pub trait SampleAngle {
    fn sample_cos_theta(&self, unitf64: UnitF64) -> Result<f64, InterpolationError>;
}
...
impl TabulatedAngularDistribution {
    ...
    ) -> Result<Self, TabulatedAngularDistributionError> {
...
#[derive(Debug, Error)]
pub enum TabulatedAngularDistributionError {
    #[error("Unsupported interpolation scheme for tabulated angular distribution: {0}")]
    InvalidInterpolationScheme(InterpolationScheme),

    #[error("cos_theta_bins ({0}) and cos_theta_cdf ({1}) must be of the same length")]
...
impl EquiprobableBinsAngularDistribution {
    pub fn new(cos_theta_bins: Vec<f64>) -> Result<Self, EquiprobableBinsAngularDistributionError> {
...
#[derive(Debug, Error)]
pub enum EquiprobableBinsAngularDistributionError {
    #[error("Expected {0} cos(theta) bin boundaries, got {1}")]
    InvalidNumberOfBins(usize, usize),
