...
use crate::ace::blocks::tyr::NumberOfExitingNeutrons;
use crate::ace::blocks::{DataBlockType, MTR, TYR, LAND};
use crate::ace::blocks::block_traits::{get_block_start, block_range_to_slice, PullFromXXS, Process};
use crate::ace::interpolation::{self, InterpolationScheme};
...
type AngularDistributionMap = HashMap<MT, EnergyDependentAngularDistribution>;
...
impl<'a> PullFromXXS<'a> for AND {
    fn pull_from_xxs_array(always_expected: bool, arrays: &'a Arrays) -> Option<&'a [f64]> {
        ...
        let last_and_entry_relative_index = LAND::pull_from_xxs_array(true, arrays)?
            .iter()
...
mod tests {
    ...
    use crate::ace::angular_distributions::{AngularDistribution, IsotropicAngularDistribution, TabulatedAngularDistribution};
