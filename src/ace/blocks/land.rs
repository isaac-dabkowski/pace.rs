...
use crate::ace::blocks::{DataBlockType, MTR, TYR};
...
impl<'a> Process<'a> for LAND {
    ...
    fn process(data: &[f64], _arrays: &Arrays, mtr: &Option<MTR>) -> Self {
        ...
        let mut angular_distribution_locs: HashMap<MT, isize> = if mtr.is_some() {
            data[1..]
                .iter()
                .enumerate()
                .map(|(i, &val)| (
...
impl LAND {
    pub fn mt_values_with_distributions(&self, tyr: &Option<TYR>) -> Vec<MT> {
        ...
        mt_vals.push(MTNumber::ElasticScattering as MT);
        mt_vals
    }
}
